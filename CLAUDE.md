...
**Batch summary.** `_report_result` accumulates per-directory tallies in `self._pending`, each stamped with a `last_seen` time. `_flush_summaries` runs on every idle `queue.Empty` tick of `_worker_loop`: for each folder that has had no new file for `SUMMARY_QUIET_SECONDS` (default 60) it drops the tally and, if it saw 2+ files, emits one `Summary [folder]: N passed, M failed …` line (console + that album's log + a notification if any failed). The quiet-period debounce is essential — a Soulseek album arrives track by track, so flushing immediately on drain would discard each folder's tally before the next track landed and no summary would ever reach the 2-file threshold. `disable()` calls it with `force=True` to flush everything unconditionally.
